...
#' Augmented ARDL(AARDL) cointegration test
#'
#' This function performs the Augmented Autoregressive Distributed Lag (AARDL) bounds test for cointegration, offering several advantages over other methods. First, it allows variables to be integrated at levels I(0), I(1), or a mix of both, without requiring variables to be integrated beyond I(1). Second, it does not necessitate the dependent variable to be I(1). Additionally, it conducts three tests to verify cointegration: a general F-test, a t-test for the dependent variable, and an F-test for independent variables. Finally, the AARDL test remains robust against small sample sizes and endogeneity issues.
#' @param model The kardl obejct
#' @param case Numeric or character. Specifies the case number for the test. Acceptable values are 1, 3, 5, and "auto". If "auto" is chosen, the function determines the case automatically based on the model.
#' @param PF The PSS F test object if available
#' @param Pt The PSS t test object if available
#' @param signif_level Character or numeric. Specifies the significance level to be used in the function.
#' Acceptable values are "auto", "0.10", "0.1", "0.05", "0.025", and "0.01".
#' If a numeric value is provided, it will be converted to a character string.
#' If "auto" is chosen, the function determines the significance level automatically.
...
#'
#'
#' @return A list containing the results of the AARDL cointegration test, including:
#' \itemize{
#' \item \code{type}: The type of test performed, which is "cointegration".
#' \item \code{case}: The case number used in the test (1, 3, 5, or "auto").
#' \item \code{statistic}: The F-statistic value calculated from the test.
#' \item \code{k}: The number of long-run variables in the model.
#' \item \code{Cont}: The conclusion of the test, indicating whether cointegration is present, inconclusive, or absent.
#' \item \code{BoundNum}: A numeric representation of the conclusion, where 1 indicates cointegration, 0 indicates inconclusive, and -1 indicates no cointegration.
#' \item \code{BoundNumAll}: A numeric representation of the conclusion for all tests, where 1 indicates cointegration, 0 indicates inconclusive, and -1 indicates no cointegration.
#' \item \code{siglvl}: The significance level used in the test, either "auto" or one of the specified numeric levels.
#' \item \code{siglvls}: A vector of significance levels used in the test for all tests.
#' \item \code{criticalValues}: A vector of critical values for the test, corresponding to the significance levels.
#' \item \code{parameter}: The names of the long-run variables in the model.
#' \item \code{Fmodel}: The linear hypothesis model used in the test.
#' \item \code{Bound}: The conclusion of the test, indicating whether cointegration is present, inconclusive, or absent.
#' \item \code{Pt}: The t-statistic value calculated from the PSS t test.
#' \item \code{PF}: The F-statistic value calculated from the PSS F test.
#' \item \code{warnings}: Any warnings generated during the test, such as sample size concerns.
#' \item \code{method}: The method used for the test, which is "AARDL".
#'
#' }
#' @seealso \code{\link{pssf}}  \code{\link{psst}}  \code{\link{banerjee}}     \code{\link{narayan}}
#' @export
#' @importFrom car linearHypothesis
#' @references McNown, R., Sam, C. Y., & Goh, S. K. (2018). Bootstrapping the autoregressive distributed lag test for cointegration. Applied Economics, 50(13), 1509-1521.
#' @references Sam, C. Y., McNown, R., & Goh, S. K. (2019). An augmented autoregressive distributed lag bounds test for cointegration. Economic Modelling, 80, 130-141.
#' @examples
#'
#'
#' kardl_model<-kardl(imf_example_data,
...
#' #' # Using magrittr
#' imf_example_data %>% kardl(CPI~ER+PPI+asym(ER)+deterministic(covid)+trend,
#'                     mode=c(1,2,3,0)) %>%  aardl() %>% summary()
aardl<-  function(model,case=3,PF=F,Pt=F,signif_level = "auto"){
  if(! case %in% c(1,3,5,"auto")){
    stop("Invalid case. The options for case are 1, 3, 5 and auto")
  }
  if (is.numeric(signif_level)) {
    signif_level <- as.character(signif_level)
  }
  ...
  if(isFALSE(PF))PF<-pssf(OptModel,case,signif_level)
  if(isFALSE(Pt))Pt<-psst(OptModel,case,signif_level)
  warningArray<-unique( c(warningArray,PF$warnings,Pt$warnings))



  # longrunNames<-paste0("L1.", inputs$longRunVars[inputs$longRunVars != inputs$dependentVar])
  longrunNames<-unique( replace_lag_var(.kardl_env$LongCoef ,inputs$longRunVars[inputs$longRunVars != inputs$dependentVar],1))

  kisit<-paste( longrunNames,"=0")

  Fmodel<-linearHypothesis(OptModel$finalModel$model ,kisit,test="F")
  SF<-Fmodel$F[2]

  n<-OptModel$finalModel$n

  obsnumber<-round((n-25)/5)
  if(obsnumber>12)obsnumber<-12


  sayi<-length(inputs$longRunVars)-1 # all longvars minus dependent
  sayi<-length(inputs$longRunVars)-1
  if(sayi>7)sayi<-7


  sutun2<-(obsnumber*15-14)+sayi*2
  sutun1<-sutun2-1
  ...
  if(case == 1){
    ...
    #   Table 1. Case I (No intercepts; no trends).
    #   N,k = 1,k = 2,k = 3,k = 4,k = 5,k = 6,k = 7,
    #   I(0),I[1],I[0],I[1],I[0],I[1],I[0],I[1],I[0],I[1],I[0],I[1],I[0],I[1])
    # p = 0.010
    out0.010 <-c(30,7.79,11.61,5.52,8.45,4.59,7.21,4.13,6.67,3.92,6.24,3.69,6.12,3.62,5.83,
                 ...
                 "above",6.65,10.00,4.65,7.13,3.75,5.93,3.37,5.35,3.05,4.85,2.84,4.55,2.65,4.31)

    out0.025 <-c(30,5.65,8.90,4.31,6.77,3.67,5.93,3.34,5.46,3.19,5.17,3.01,5.07,2.94,4.96,
                 ...
                 "above",5.10,8.05,3.73,5.99,3.12,5.05,2.82,4.57,2.58,4.27,2.44,4.00,2.30,3.86)

    out0.050 <-c(30,4.19,7.07,3.40,5.59,3.00,4.96,2.75,4.61,2.62,4.41,2.53,4.34,2.48,4.22,
                 ...
                 "above",3.87,6.57,3.02,5.05,2.61,4.41,2.39,4.01,2.22,3.81,2.12,3.60,2.02,3.47)

    out0.100 <-c(30,2.88,5.13,2.53,4.42,2.32,3.99,2.18,3.78,2.12,3.67,2.04,3.60,2.01,3.53,
                 ...
                 "above",2.72,5.00,2.32,4.09,2.09,3.69,1.95,3.46,1.85,3.30,1.77,3.17,1.72,3.06)
  }else if(case %in% c(2,3)){
    #   Table 2. Case III (Unrestricted intercepts; no trends).
    #   N,k = 1,k = 2,k = 3,k = 4,k = 5,k = 6,k = 7,
    #   I(0),I(1),I(0),I(1),I(0),I(1),I(0),I(1),I(0),I(1),I(0),I(1),I(0),I(1))
    # p = 0.010,
    out0.010 <-c(30,7.70,12.34,5.41,9.16,4.60,7.72,4.15,6.83,3.94,6.48,3.72,6.16,3.62,6.12,
                 ...
                 "above",6.53,11.05,4.71,7.68,3.83,6.33,3.33,5.47,3.05,5.02,2.84,4.69,2.62,4.42)

    out0.025 <-c(30,5.61,9.56,4.25,7.24,3.64,6.22,3.36,5.60,3.19,5.33,3.04,5.18,2.96,5.06,
                 ...
                 "above",4.95,8.84,3.74,6.42,3.15,5.40,2.79,4.73,2.59,4.41,2.43,4.14,2.29,3.97)

    out0.050 <-c(30,4.22,7.51,3.36,5.90,2.96,5.14,2.80,4.70,2.65,4.54,2.53,4.37,2.47,4.30,
                 ...
                 "above",3.79,7.21,3.01,5.42,2.62,4.65,2.39,4.18,2.24,3.90,2.12,3.72,2.01,3.58)

    out0.100 <-c(30,2.88,5.34,2.53,4.54,2.30,4.11,2.22,3.84,2.12,3.76,2.06,3.62,2.02,3.55,
                 ...
                 "above",2.67,5.31,2.31,4.33,2.08,3.86,1.96,3.58,1.86,3.39,1.78,3.25,1.72,3.15)

    case <- 3 # Default case is 3, which means unrestricted intercept and no trend
  }else if(case %in% c(4,5)){
    #   Table 3. Case V (Unrestricted intercepts; unrestricted trends).
    #   n,k = 1,k = 2,k = 3,k = 4,k = 5,k = 6,k = 7,
    #   I(0),I(1),I(0),I(1),I(0),I(1),I(0),I(1),I(0),I(1),I(0),I(1),I(0),I(1))
    # p = 0.010,
    out0.010 <-c(30,7.82,12.57,5.46,9.05,4.67,7.58,4.30,7.01,4.00,6.66,3.77,6.25,3.70,6.00,
                 ...
                 "above",6.65,11.76,4.58,7.94,3.86,6.46,3.40,5.68,3.05,5.12,2.81,4.79,2.68,4.54)

    out0.025 <-c(30,5.55,9.51,4.22,7.12,3.17,6.16,3.37,5.67,3.24,5.37,3.05,5.12,3.02,5.01,
                 ...
                 "above",5.02,9.19,3.67,6.53,3.17,5.45,2.81,4.94,2.60,4.51,2.41,4.23,2.31,4.04)

    out0.050 <-c(30,4.22,7.33,3.35,5.77,2.97,5.09,2.74,4.71,2.67,4.49,2.56,4.34,2.54,4.24,
                 ...
                 "above",3.87,7.25,3.00,5.45,2.64,4.70,2.40,4.31,2.24,3.98,2.11,3.75,2.02,3.62)

    out0.100 <-c(30,2.91,5.20,2.53,4.43,2.30,4.04,2.18,3.79,2.13,3.67,2.07,3.56,2.06,3.48,
                 ...
                 "above",2.74,5.30,2.30,4.34,2.11,3.89,1.97,3.62,1.87,3.44,1.78,3.27,1.72,3.17)

    case <- 5 # Default case is 5, which means unrestricted intercept and trend
  }


  CrTable<-as.numeric(c(sayi,n,out0.100[sutun1], out0.100[sutun2], out0.050[sutun1], out0.050[sutun2], out0.025[sutun1], out0.025[sutun2], out0.010[sutun1], out0.010[sutun2]))
  names(CrTable)<-c("k","N","0.10L","0.10U","0.05L","0.05U","0.025L","0.025U","0.01L","0.01U")


  if(signif_level == "auto"){
    Bound<-"Cointegration";AARDLBoundNum=1
    if(SF>=CrTable["0.01U"]){
      sig<-"0.01"
    }else if(SF>=CrTable["0.025U"]){
      sig<-"0.025"
    }else if(SF>=CrTable["0.05U"] ){
      sig<-"0.05"
    }else if(SF>=CrTable["0.10U"]  ){
      sig<-"0.10"
    }else{
      sig<-""
      if(SF>=CrTable["0.10L"] ){
        Bound<-"Inconclusive";AARDLBoundNum=0
      }else{
        Bound<-"No Cointegration";AARDLBoundNum=-1
      }
    }
  }else{
    sig<-signif_level
    UpLimit<-paste0(sig,"U")
    LowLimit<-paste0(sig,"L")
    if(SF>=CrTable[UpLimit]){
      Bound<-"Cointegration ";AARDLBoundNum=1
    }else if(SF>=CrTable[LowLimit]){
      Bound<-"Inconclusive";AARDLBoundNum=0
    }else{Bound<-"No Cointegration";AARDLBoundNum=-1
    }
  }

  if(PF[["BoundNum"]]==1){
    if(AARDLBoundNum ==1 && Pt[["BoundNum"]]==1){
      AARDLtxt="Cointegration"
      AARDLBoundNumTotal=1
    }else{
      AARDLtxt="Degernerate Cointegration"
      AARDLBoundNumTotal=0
    }
  }else{
    AARDLtxt="No Cointegration"
    AARDLBoundNumTotal=-1
  }
  ...
  kardlList<-list(
    ...
    k=sayi,
    Cont=Bound,
    BoundNum=AARDLBoundNum,
    BoundNumAll=AARDLBoundNumTotal,
    siglvl=max(PF$siglvl,Pt$siglvl,sig),
    siglvls=c("PSSF"=PF$siglvl,"PSSt"=Pt$siglvl,"SMG"=sig),
    criticalValues=CrTable ,
    parameter =longrunNames,
    Fmodel=Fmodel,
    Bound=AARDLtxt,
    Pt=Pt,
