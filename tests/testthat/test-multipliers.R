...
#' @srrstats {G5.9a} Bootstrap confidence intervals are stable under trivial
#'  noise perturbations: adding machine-epsilon noise to the data should not
#'  meaningfully change point estimates or confidence interval bounds.
#' @srrstats {G5.9b} Bootstrap results are stable across different random seeds:
#'  point estimates should be identical, and confidence interval widths should
...
test_that("bootstrap results are stable across different seeds", {
  ...
  expect_true(
    ...
    label = paste0(
      ...
      "Max seed-to-seed diff: ",
      round(max(diff_of_widths), 6),
      ", 25% of CI width range: ",
      round(0.25 * range_of_widths, 6)
    )
  )
})
...
test_that("Adding machine-epsilon noise to data does not change results", {
  ...
  # This ensures the perturbation stays at machine-epsilon *relative*
  # scale even after cumulative summation inside kardl's asymmetric
  # decomposition.
  col_scales <- sapply(
    data_noisy[, numeric_cols, drop = FALSE],
  ...
  noise_matrix <- matrix(
    rnorm(
      sum(numeric_cols) * nrow(data_noisy),
      mean = 0,
