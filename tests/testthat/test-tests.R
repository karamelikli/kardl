...
#' component, confirming short_wald_summary is NULL and long_wald_summary is
#'  populated.
test_that("symmetrytest with component = 'longrun' only works", {
  ...
  expect_false(is.null(st$long_wald_summary))
  expect_null(st$short_wald_summary)
})
#' @srrstats {G5.7} Tests symmetry test when restricted to the short-run
#' component, confirming long_wald_summary is NULL and short_wald_summary is
#' populated.
test_that("symmetrytest with component = 'shortrun' only works", {
  ...
  expect_s3_class(st, "kardl_symmetric")
  expect_null(st$long_wald_summary)
  expect_false(is.null(st$short_wald_summary))
})
#' @srrstats {G5.7} Tests that chi-squared test type produces a Pr(>Chisq)
#' column in the long_wald_summary table.
test_that("symmetrytest with type = 'Chisq' works", {
  ...
  expect_true("Pr(>Chisq)" %in% colnames(st$long_wald_summary))
})
#' @srrstats {G5.7} Confirms that when `vars` is restricted to a single
#' variable, only that variable appears in the long_wald_summary row names.
...
#' @srrstats {G5.8} Verifies edge-condition handling for invalid variable
#'  selection.
...
test_that("pssf with specific signif_level works", {
  kardl_reset()
  model <- kardl(DriversKilled ~ PetrolPrice + drivers + trend, Seatbelts,
    mode = c(1, 1, 1)
  )
...
#' @srrstats {G5.2b} Checks that passing an ECM object to `narayan()` is
#' rejected.
...
#' @srrstats {G5.7} Tests that the `min_prob` threshold filters coefficients
#' and returns a `kardl_mplier` object with the correct number of rows.
#' @srrstats {TS4.3} Verifies the horizon index dimension of the returned
#' mpsi matrix.
...
test_that("bootstrap with asymmetric model works", {
  kardl_reset()
  model <- kardl(DriversKilled ~ asymmetric(PetrolPrice), Seatbelts,
    mode = c(1, 1, 1)
  )
...
#' @srrstats {G5.2b} Confirms that a plain `lm` object is rejected
#' by `bootstrap()`.
